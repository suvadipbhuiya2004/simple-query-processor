...
public:
    ...
    SeqScanNode(std::string t, std::string qualifier) : PlanNode(PlanType::SEQ_SCAN), table(std::move(t)), outputQualifier(std::move(qualifier)) {}
};
...
public:
    ...
    JoinNode(JoinType t, JoinAlgorithm algo, std::unique_ptr<Expr> cond) : PlanNode(PlanType::JOIN), joinType(t), algorithm(algo), condition(std::move(cond)) {}
};
...
public:
    ...
    explicit FilterNode(std::unique_ptr<Expr> pred) : PlanNode(PlanType::FILTER), predicate(std::move(pred)) {}
};
...
public:
    ...
    explicit ProjectionNode(std::vector<std::unique_ptr<Expr>> cols) : PlanNode(PlanType::PROJECTION), columns(std::move(cols)) {}
};
...
public:
    ...
    AggregationNode(std::vector<std::unique_ptr<Expr>> groupExprs_, std::unique_ptr<Expr> havingExpr_) : PlanNode(PlanType::AGGREGATION), groupExprs(std::move(groupExprs_)), havingExpr(std::move(havingExpr_)) {}
};
...
public:
    ...
    explicit SortNode(std::string col) : PlanNode(PlanType::SORT), orderByColumn(std::move(col)) {}
};
