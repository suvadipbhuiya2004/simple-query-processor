...
namespace{
    ...
    std::vector<std::string> deriveQualifiedColumns(const PlanNode *plan, Database &db){
        ...
        case PlanType::SEQ_SCAN:{
            ...
            const auto baseColumns = getTableColumns(db, node->table);
...
std::unique_ptr<Executor> ExecutorBuilder::build(const PlanNode *plan, Database &db){
    ...
    case PlanType::SEQ_SCAN: {
        ...
        return std::make_unique<SeqScanExecutor>(
            &db.getTable(node->table),
            schema,
            node->outputQualifier);
    }
    ...
    case PlanType::JOIN:{
        ...
        return std::make_unique<JoinExecutor>(
            ...
            node->joinType,
            node->algorithm,
            node->condition ? node->condition->clone() : nullptr,
            std::move(leftColumns),
            std::move(rightColumns));
    }
    ...
    case PlanType::LIMIT: {
        ...
        return std::make_unique<LimitExecutor>(
            std::move(child), static_cast<std::size_t>(node->limitCount));
    }
